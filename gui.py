...
class Application:
    ...
    def afficher_ecran_connexion(self):
        ...
        # Boutons
        ttk.Button(frame, text="Se connecter", command=self.connexion).pack(
            fill="x", pady=5
        )
        ttk.Button(
            frame,
            text="Créer un compte",
            command=self.afficher_ecran_inscription).pack(
            fill="x")
    ...
    def afficher_ecran_inscription(self):
        ...
        ttk.Button(
            frame,
            text="Retour",
            command=self.afficher_ecran_connexion).pack(
            fill="x")
    ...
    def afficher_ecran_principal(self):
        ...
        ttk.Label(
            menu_frame,
            text=f"Bienvenue, {self.utilisateur_connecte['username']}",
            font=("Helvetica", 12),
        ).pack(side="left")
        ttk.Button(
            menu_frame,
            text="Déconnexion",
            command=self.deconnexion).pack(
            side="right")
        ...
        ttk.Button(btn_frame, text="Ajouter", command=self.popup_ajouter).pack(
            side="left", padx=5
        )
        ttk.Button(
            btn_frame,
            text="Modifier",
            command=self.popup_modifier).pack(
            side="left",
            padx=5)
        ttk.Button(
            btn_frame,
            text="Supprimer",
            command=self.supprimer_selection).pack(
            side="left",
            padx=5)
        ttk.Button(
            btn_frame,
            text="Actualiser",
            command=self.rafraichir_liste).pack(
            side="left",
            padx=5)
