...
def executer_bandit():
    ...
    rapport = f"{DOSSIER_RAPPORTS}/bandit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
def executer_pylint():
    ...
    creer_dossier_rapports()
    rapport = f"{DOSSIER_RAPPORTS}/pylint_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
def executer_safety():
    ...
    creer_dossier_rapports()
    rapport = f"{DOSSIER_RAPPORTS}/safety_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                    ...
                    for pattern, description in patterns_dangereux:
                        if pattern in ligne_lower and "=" in ligne and '\"' in ligne:
                            if not ligne.strip().startswith("#"):
                                problemes.append(f"{fichier}:{i} - {description} potentiel")
